...
void SaveFrame(AVFrame *pFrame, int width, int height, int iFrame) {
    ...
    // Open file
    sprintf(szFilename, "/Users/yuming/Movies/decode/frame%d.ppm", iFrame);
...
#define BUFFER_SIZE 4096
    ...
    // This code flushes the parse in two case:
    ...
    if (feof(file)) {
        av_parser_parse2(pVCodecParserCtx, pVCodecCtx, &pPacket->data, &pPacket->size,
                                       NULL, 0, 0, 0, 0);
        if (pPacket->size) {
            fprintf(stderr, "flush the parser.\n");
            fprintf(stderr, "packet size = %d\n", pPacket->size);
            return 1;
        }
...
int SodtpReadPacket(
    ...
    fprintf(stderr, "size1 = %d,\t size2 = %d,\t compare value = unknown\n",
            pBlock->size, pCmpPkt->size);
    ...
    //
    // if (pBlock) {
    // if (ret != SodtpJitter::STATE_BUFFERING) {
    if (ret == SodtpJitter::STATE_NORMAL) {
        if (av_packet_from_data(pPacket, pBlock->data, pBlock->size) < 0) {
            fprintf(stderr, "Fail to create AVPacket\n");
        }
    } else {
    ...
    int cmp_val = (0 == memcmp(pPacket->data, pCmpPkt->data, pBlock->size));
    fprintf(stderr, "size1 = %d,\t size2 = %d,\t compare value = %s\n",
            pPacket->size, pCmpPkt->size,
            cmp_val?"same":"different");
    //
    //
    //
    return ret;
...
// @deprecated
AVFormatContext* sniff_format(uint8_t* pbuf, int nbufsize, double* dTime)
{
    // av_register_all();
    AVIOContext *pb = NULL;
    ...
    // ERROR! Segment fault here.
    // So we do not use this sniff_format() function any more.
...
// @deprecated
int video_viewer2(SodtpJitterPtr pJitter) {
    ...
    ////
    //
    AVFormatContext *cmp_fmt = NULL;
    AVPacket cmp_pkt;
    bool cmp_val  = false;
    init_resource(&cmp_fmt, "/Users/yuming/Movies/bigbuckbunny/480x272_3.h265");
    //
    ////
    ...
    // file_read_packet2(cmp_fmt, &cmp_pkt);
    // cmp_val = compare_packet(&packet, &cmp_pkt);
    // printf("size1 = %d,\t size2 = %d,\t compare value = %s\n", 
    //     packet.size, cmp_pkt.size,
    //     cmp_val?"same":"different");
    // return 1;

    av_init_packet(&packet);
    // ret = SodtpReadPacket(pJitter, &packet, pBlock);
    while (true) {
        printf("sleep once!\n");
        usleep(20000);
        ret = SodtpReadPacket(pJitter, &packet, pBlock);
    ...
    printf("stream %d,\t block %d,\t size %d\n", pBlock->stream_id, pBlock->block_id, pBlock->size);
    pVFormatCtx = sniff_format2(packet.data, packet.size);
    ...
    iFrame = 1;
    while (true) {
        while (ret == SodtpJitter::STATE_BUFFERING) {
            printf("sleep once!\n");
            usleep(20000);
            ret = SodtpReadPacket(pJitter, &packet, pBlock);
            if (pJitter->state == SodtpJitter::STATE_CLOSE) {
        ...
        if (ret == SodtpJitter::STATE_NORMAL) {
            printf("packet %d,\t size %d,\t id %d\n", iFrame, packet.size, pBlock->block_id);
            DecodePacket(pVCodecCtx, pSwsCtx, pFrame, pFrameRGB, &packet, pJitter->stream_id, pBlock->block_id+1);
            printf("debug: %s:%s:%d\n", __FILE__, __FUNCTION__, __LINE__);
            iFrame++;
        }
        // STATE_SKIP
        else {
            printf("skip this block.\n");
        }
        // Free the packet that was allocated by av_read_frame
        printf("debug: %s:%s:%d\n", __FILE__, __FUNCTION__, __LINE__);
        // av_free_packet(&cmp_pkt);
